...
Trước tích hợp, orchestrator phải kiểm tra diff thật giữa `main` và branch: mọi đường dẫn đều thuộc `write_scope`, và artifact được bảo vệ phải có đúng scope tệp và đang là `agent-draft` trên `main`. Orchestrator push branch lên remote, tạo Pull Request trên GitHub và merge PR (tương đương merge commit). Sau khi PR được merge, orchestrator cập nhật `done`, chuyển các artifact vừa tích hợp sang `human-editing`, commit metadata tích hợp, rồi mới gọi `cleanup-worktree`. Mọi ngoại lệ phải được ghi trong handoff.
