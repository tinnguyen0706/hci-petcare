...
3. **Font chữ, Typography & Chống Tràn Chữ (Tuân thủ rules/layout-and-typography-rules.md):**
   ...
   - **Chống đè chữ cùng hàng**: Khi có 2 text cùng tọa độ $y$ (trái $x=38$, phải $x=392$), text trái tối đa 26 ký tự, text phải tối đa 16 ký tự, chừa gap $\ge 24\text{px}$.
   - **Bắt buộc ngắt dòng**: Khi văn bản dài hơn giới hạn trên, phải tách thành nhiều thẻ `<text>` riêng biệt với $\Delta y \ge 19\text{px}$.
...
4. **Bảng màu Design Tokens chuẩn:**
   ...
   - *Accent & Cảnh báo*: Đỏ dị ứng `#9F1239` (Nền `#FFF1F2`, viền `#FECDD3`)
...
5. **Quy chuẩn Biểu tượng (Iconography) — TUYỆT ĐỐI KHÔNG DÙNG EMOJI MÀU MÈ:**
   ...
   - Thay thế 100% bằng **Biểu tượng Vector Đơn sắc (Monochrome Vector)** hoặc **Ký tự Hình học phẳng Tối giản**:
     - *Avatar thú cưng*: Vòng tròn viền teal kèm text chữ viết tắt in hoa (Ví dụ: `BƠ` hoặc `ML`).
     - *Nút Back / Tiếp tục*: `<text font-size="20" font-weight="bold">‹</text>` hoặc `›`.
     - *Dấu xác nhận / Đóng*: `<text font-size="14" font-weight="bold">✓</text>` hoặc `✕`.
     - *Thẻ Tag / Cảnh báo*: Dùng nhãn chữ rõ ràng (Text Badges) như `[DA NHẠY CẢM]`, `[PHÒNG CÁCH LY]`, `[KHUYÊN DÙNG]`, `[ĐÃ TIÊM PHÒNG]`.
...
* **Status Bar:** Giờ `14:00`, chỉ báo `5G • 100%` tối giản.
...
* **Content / Service Card:** Khối bo góc `rx="16"`, viền phân cách, tag chữ nổi bật (Badge) và giá tiền.
* **Time Slot Picker:** Lưới chọn khung giờ đặt hẹn với viền và nền trạng thái.
* **Input / Search Box:** Khung nhập liệu kèm placeholder.
* **Action Button:** Nút bấm Full-width bo góc `rx="16"` với màu chủ đạo.
* **Bottom Navigation Bar:** 4 tab điều hướng chính kèm nhãn trang và thanh Home Indicator chuẩn iPhone 14 Pro Max.
...
builder.add_card("Combo Tắm & Cắt Tỉa", subtitle="Bé Bông (Poodle)", price="250.000đ", height=120)
...
3. **Kiểm thử Bắt buộc:** Thực hiện 4 bước kiểm thử tại `rules/layout-and-typography-rules.md`.
4. **Lưu file:** Ghi file vào thư mục `deliverables/<tên-màn-hình>.svg`.
5. **Hướng dẫn người dùng:** Thông báo đường dẫn file và hướng dẫn kéo thả trực tiếp vào Figma Canvas để sử dụng ngay.
