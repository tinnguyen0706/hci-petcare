...
- **Quy chuẩn bố cục & kiểu chữ**: Tuân thủ `rules/layout-and-typography-rules.md`.
...
- **Bộ tệp bản vẽ SVG Interactive Frame chuẩn Figma** tương ứng với từng bước trong kịch bản đó (ví dụ: `01_screen_name.svg`, `02_screen_name.svg`...).
...
- *(Lưu ý: Tuyệt đối KHÔNG tạo tệp `.html`)*.
...
graph TD
    ...
    Step4 --> Step5["5. Biên soạn Interaction Spec (interaction-spec.md)"]
...
4. **Bước 4 — Gọi Subagent `figma-agent`**:
   - Chuyển giao đặc tả cho `figma-agent` để sinh mã SVG vector chuẩn Figma cho từng màn hình, áp dụng đúng Design Tokens, nhóm `<g id="...">` ngữ nghĩa và tuân thủ `rules/layout-and-typography-rules.md`.
5. **Bước 5 — Biên soạn Interaction Spec (Ma trận nối dây tương tác)**:
   - Xuất tài liệu `interaction-spec.md` mô tả ma trận nối dây Prototype (Trigger, Transition: Smart Animate 300ms, Hotspots).
